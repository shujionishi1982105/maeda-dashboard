...
2. スマホ・PCのブラウザで本番URLのログイン画面を開き、画面最上部（見出しの上）に不自然な余白が出ていないか確認する（Task 3のコードレビューで、`inject_pwa_head()`のscriptタグ挿入がFlexboxのgapにより微小な空白を生む可能性が理論上指摘されており、この環境ではヘッドレスブラウザがなく検証できなかったため、実機での確認が必要）
